...
for j in range(20, max_size_ind):
    
# ---------------- GENERATE DATA ------------------------- #
    
    size = data_sizes[j]
    x, y, z, f = tq.BatchData(0, size)
    dr.ReconstructFile(0, size, conc=False, entr=False)
    
    xV, yV, zV, fV = tq.BatchData(1, val_sizes[j])
    dr.ReconstructFile(1, val_sizes[j], conc=False, entr=False)
    
    xT, yT, zT, fT = tq.BatchData(2, test_sizes[j])            
...
# ---------------- RECORD DATA STATS ------------------------- #
    ...
    filename = "PURE_" + "DATA_AVGS" + str(size) + ".csv"        
    ...
    filename = "PURE_" + "DATA_FULL" + str(size) + ".csv"        
