...
def ReplaceMe(batch_size, train_size, train_batch, valid_size, valid_batch, test_size, test_batch, val_indices):
    ...
    loss, loss_array, fid, fid_array = NN.TrainTestNet(batch_size, train_size, train_batch, valid_size, valid_batch, test_size, test_batch, val_indices)
...
for a in range(3, 4):
    ...
    flag = True
    while(flag):    
        val = np.unique(np.random.randint(low=0, high=16, size=(4)))
        if(len(val) == 4):
            flag = True
            break
...
# ---------------- CONCURRENCE ------------------------- #
    ...
    # ---------------- GENERATE DATA ------------------------- #
            ...
            xV, yV, zV, fV, cV, sV = tq.BatchDataMixed(1, val_sizes[j], select_concurrence = [True, i/6, (i+1)/6])
            dr.ReconstructFile(1, val_sizes[j], conc=True, entr=False)
            
            xT, yT, zT, fT, cT, sT = tq.BatchDataMixed(2, test_sizes[j], select_concurrence = [True, i/6, (i+1)/6])            
            dr.ReconstructFile(2, test_sizes[j], conc=True, entr=False)
    ...
    # ---------------- RECORD DATA STATS ------------------------- #
            ...
            averages = [x,y,z,xV,yV,zV,xT,yT,zT,fid_avg1,fid_avg2,fid_avg3,loss_avg1,loss_avg2,loss_avg3,fid_avg, fid_std, loss_avg, loss_std, val]
            avg_columns = ["Train_Avg_Fid","Train_Data_Size","Train_StdDev_Fid",\
                       ...
                       "NN_Test_Avg_Fid", "NN_Test_Std_Fid", "NN_Test_Avg_MAE", "NN_Test_Std_MAE", "Val_Indices" ]
                
            da = pd.DataFrame(data = averages)
            da = da.T
            da.columns = avg_columns
            filename = str(a) + "_4E_" + "DATA_AVGS" + str(size) +"CONC" + str(i) + "-6.csv"        
            ...
            filename = str(a) + "_4E_" + "DATA_FULL" + str(size) +"CONC" + str(i) + ".csv"        
            ...
            filename = str(a) + "_4E_" + "DATA_CONC" + str(size) +"CONC" + str(i) + ".csv"        
